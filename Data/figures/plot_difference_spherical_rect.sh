...
depth=$1
input_file_rec='../ascii/merged_'$depth'_rectangular.xyz'
input_file_sph='../ascii/merged_'$depth'_spherical.xyz'
output_file='./diff_'$depth'km.ps'
...
gmt makecpt -I -T-0.05/0.05/0.001 -D -Z -C"panoply_white.cpt" > "tomo.cpt"

# gmt surface -R$range_global -H1 -I0.5 $input_file -Goutgrid.grd
gmt surface -R$range_regional -I0.5 $input_file_rec -Goutgrid_rec.grd
gmt surface -R$range_regional -I0.5 $input_file_sph -Goutgrid_sph.grd

gmt grdmath outgrid_sph.grd outgrid_rec.grd SUB = diff.grd
gmt grdimage -R -J$frame_plot diff.grd -Bp20WSen:."depth ${depth} km": -C$dir_home"tomo.cpt" -Y5c -X3c -K > $output_file
gmt pscoast -R -J -Dl -W0.03c,black -A5000 -O -K >> $output_file
gmt psxy -R -J "./mask.xyz" -W0.03c,purple,-. -O -K >> $output_file
gmt psscale -D10/-1c/5c/0.25ch -C"./tomo.cpt" -B0.05:"dlnVs (%)": -O >> $output_file

rm *.grd tomo.cpt panoply_white.cpt gmt.history mask.xyz
