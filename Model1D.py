...
class Model1D:
    ...
    def get_header(self):
        ...
        if not self.populated:
            raise Exception(
                "Call to get_header() when the model has not been populated"
            )
    ...
    def __init_interpolation(self):
        ...
        self.interpolate = [
            interp1d(
                self.table[lower_bound : upper_bound + 1, 0].T,
        ...
        for ir in range(radii.size):
            ...
            if radii[ir] in self.radius_discon:
                ...
                if eval_discon_above[ir]:
                    result[ir, :] = self.table[
                        self.index_discon[radii[ir] == self.radius_discon] + 1, :
                    ]
                else:
                    result[ir, :] = self.table[
                        self.index_discon[radii[ir] == self.radius_discon], :
                    ]
            else:
